...
class ReportGenerator(FPDF):
    ...
    def _escrever_nota_tecnica_local(self):
        ...
        regras = (
            "* Regra de Sobressinal (Mp): Alerta de agressividade se Mp > 20%; "
            "Moderado entre 0.5% e 20%; Conservador se ~0%.\n"
            "* Regra de Estabilidade (ts): Se o tempo de acomodacao for nulo (N/A), "
            "o sistema e diagnosticado como instavel.\n"
            "* Regra de Erro Estatico (ess): Se ess > 0.05, indica que a acao integral (Ti) "
            "foi ineficaz para anular o desvio."
        )
    ...
    def _gerar_analise_estatica(self, historico):
        ...
        for nome, dados in historico.items():
            ...
            if met.overshoot is not None:
                if met.overshoot > 20:
                    txt += f"Comportamento agressivo (Mp={met.overshoot:.1f}%). "
                elif met.overshoot > 0.5:
                    txt += f"Resposta moderada (Mp={met.overshoot:.1f}%). "
                else:
                    txt += "Resposta conservadora/suave. "
            else:
                txt += "Sistema instavel. "
            if met.settling_time is not None:
                txt += f"Estabiliza em {met.settling_time:.1f}s. "
            else:
                txt += "Nao estabilizou no tempo limite. "
    ...
    def obter_analise_ia(self, simulacoes, K, tau, theta):
        """Análise via Google Gemini API — usa a nova biblioteca google-genai."""
        if not self.api_key:
            return self._gerar_analise_estatica(simulacoes)
        try:
            # Monta resumo das simulações
            resumo = ""
            ...
            prompt = (
                ...
                "NAO use saudacoes ou apresentacoes em hipotese alguma. "
                "Fale de polos, zeros e estabilidade. "
                f"Planta: K={K:.2f}, tau={tau:.2f}, atraso={theta:.2f}. "
                f"Analise detalhadamente estas simulacoes de PID: {resumo}. "
                "Conclua rigorosamente qual metodo e mais seguro para instalacoes industriais "
                "baseando-se na reducao de oscilacao."
            )
            ...
            # Tenta modelos em ordem de preferência (fallback automático)
            modelos = [
                "gemini-2.5-flash",
            ...
            return (
                f"ERRO NA COMUNICACAO COM API: {str(e)}\n\n"
    ...
    def generate(self, output_path, use_ai=True):
        ...
        melhor_id = (
            "Sundaresan"
            if self.controller.dados_identificacao['Sundaresan']['eqm']
               < self.controller.dados_identificacao['Smith']['eqm']
            else "Smith"
        )
        ...
        # 2. Tabela de Sintonia
        self.ln(5)
        self.set_font('Arial', 'B', 12)
        self.cell(0, 10, '2. Tabela de Sintonia e Desempenho Transitorio', ln=True)
        self.set_font('Courier', 'B', 8)
        self.cell(0, 6,
            f"{'Metodo Utilizado':25} | {'Kp':7} | {'Ti':7} | {'Td':7} || "
            f"{'tr(s)':8} | {'ts(s)':8} | {'Mp(%)':8}",
            ln=True,
        )
        ...
        for nome, dados in self.controller.historico_simulacoes.items():
            ...
            self.cell(0, 6,
                f"{nome:25} | {pid.Kp:7.3f} | {pid.Ti:7.3f} | {pid.Td:7.3f} || "
                f"{tr:8} | {ts:8} | {mp:8}",
                ln=True,
            )
        ...
        self.set_font('Arial', '', 10)
        analise = (
            self.obter_analise_ia(self.controller.historico_simulacoes, K, tau, theta)
            if use_ai
            else self._gerar_analise_estatica(self.controller.historico_simulacoes)
        )
        self.multi_cell(0, 5, analise)

        # 5. Apêndice (só no modo local)
        if not use_ai:
